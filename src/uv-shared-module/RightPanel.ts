...
export class RightPanel extends BaseExpandPanel {
    ...
    init(): void{
        ...
        Dispatcher.subscribe(BaseEvents.TOGGLE_EXPAND_RIGHT_PANEL, () => {
    ...
    toggleFinish(): void {
        ...
        if (this.isExpanded) {
            Dispatcher.publish(BaseEvents.OPEN_RIGHT_PANEL);
        } else {
            Dispatcher.publish(BaseEvents.CLOSE_RIGHT_PANEL);
        }
