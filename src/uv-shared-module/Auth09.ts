...
export class Auth09 {
                ...
                Auth09.handleExternalResourceResponse).then((r: Manifesto.IExternalResource[]) => {
                    ...
                    switch(error.name) {
                        case manifesto.StatusCodes.AUTHORIZATION_FAILED.toString():
                            Dispatcher.publish(BaseEvents.LOGIN_FAILED);
                            break;
                        case manifesto.StatusCodes.FORBIDDEN.toString():
                            Dispatcher.publish(BaseEvents.FORBIDDEN);
                            break;
                        case manifesto.StatusCodes.RESTRICTED.toString():
                            // do nothing
                            break;
                        default:
                            Dispatcher.publish(BaseEvents.SHOW_MESSAGE, [error.message || error]);
                    }
            });
        });
    }
    ...
    static clickThrough(resource: Manifesto.IExternalResource): Promise<void> {
        return new Promise<void>((resolve) => {
            ...
            Dispatcher.publish(BaseEvents.SHOW_CLICKTHROUGH_DIALOGUE, [{
                resource: resource,
                acceptCallback: () => {
                    ...
                    if (resource.clickThroughService) {
                        ...
                        const pollTimer: number = window.setInterval(() => {
                            if (win && win.closed) {
                                ...
                                Dispatcher.publish(BaseEvents.CLICKTHROUGH);
                                resolve();
                            }
                        }, 500);
                    }
    ...
    static restricted(resource: Manifesto.IExternalResource): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            ...
            Dispatcher.publish(BaseEvents.SHOW_RESTRICTED_DIALOGUE, [{
                resource: resource,
                acceptCallback: () => {
                    Dispatcher.publish(BaseEvents.LOAD_FAILED);
                    reject(resource);
                }
            }]);
        });
    ...
    static login(resource: Manifesto.IExternalResource): Promise<void> {
        return new Promise<void>((resolve) => {
            ...
            Dispatcher.publish(BaseEvents.SHOW_LOGIN_DIALOGUE, [{
                resource: resource,
                loginCallback: () => {
                    if (resource.loginService) {
                        ...
                        const pollTimer: number = window.setInterval(function () {
                            if (win && win.closed) {
                                window.clearInterval(pollTimer);
                                Dispatcher.publish(BaseEvents.LOGIN);
                                resolve();
                            }
                        }, 500);
                    }
                ...
                logoutCallback: () => {
                    if (resource.logoutService) {
                        ...
                        const pollTimer: number = window.setInterval(function () {
                            if (win && win.closed) {
                                window.clearInterval(pollTimer);
                                Dispatcher.publish(BaseEvents.LOGOUT);
                                resolve();
                            }
                        }, 500);
                    }
    ...
    static handleExternalResourceResponse(resource: Manifesto.IExternalResource): Promise<any> {
        ...
        return new Promise<any>((resolve, reject) => {
            ...
            if (resource.status === HTTPStatusCode.OK) {
                ...
                resolve(resource);
                Dispatcher.publish(BaseEvents.RESOURCE_DEGRADED, [resource]);
            } else {
                ...
                if (resource.error.status === HTTPStatusCode.UNAUTHORIZED ||
                    ...
                    if (!Modernizr.cors) {
                        ...
                        Dispatcher.publish(BaseEvents.SHOW_INFORMATION, [informationArgs]);
                        resolve(resource);
                    } else {
