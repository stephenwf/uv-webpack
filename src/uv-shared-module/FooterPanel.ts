...
export class FooterPanel extends BaseView {
    ...
    create(): void {
        ...
        Dispatcher.subscribe(BaseEvents.METRIC_CHANGED, () => {
            this.updateMinimisedButtons();
            this.updateMoreInfoButton();
        });

        Dispatcher.subscribe(BaseEvents.SETTINGS_CHANGED, () => {
        ...
        this.$openButton.onPressed(() => {
            Dispatcher.publish(BaseEvents.OPEN);
        });

        this.$feedbackButton.onPressed(() => {
            Dispatcher.publish(BaseEvents.FEEDBACK);
        });

        this.$bookmarkButton.onPressed(() => {
            Dispatcher.publish(BaseEvents.BOOKMARK);
        });

        this.$shareButton.onPressed(() => {
            Dispatcher.publish(BaseEvents.SHOW_SHARE_DIALOGUE, [this.$shareButton]);
        });

        this.$embedButton.onPressed(() => {
            Dispatcher.publish(BaseEvents.SHOW_EMBED_DIALOGUE, [this.$embedButton]);
        });

        this.$downloadButton.onPressed(() => {
            Dispatcher.publish(BaseEvents.SHOW_DOWNLOAD_DIALOGUE, [this.$downloadButton]);
        });

        this.$moreInfoButton.onPressed(() => {
            Dispatcher.publish(BaseEvents.SHOW_MOREINFO_DIALOGUE, [this.$moreInfoButton]);
        });
